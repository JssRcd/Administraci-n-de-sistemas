...
while true; do
    ...
    case $op in
        ...
        3) # AMBITO (Corregido)
            ...
            # Primero configuramos la IP estática del servidor
            read -p "IP para este Servidor DHCP (ej: 192.168.20.2): " srvIP
            
            if ! validar_ip "$srvIP"; then
                echo -e "\e[31mIP del servidor invalida\e[0m"
                read -p "Presione Enter para continuar..."
                continue
            fi
            
            echo -e "\n\e[33m[i] Configurando IP estatica en el servidor...\e[0m"
            ...
            echo -e "\e[32m[+] IP del servidor configurada: $srvIP\e[0m"
            
            # Ahora configuramos el rango DHCP
            echo ""
            read -p "IP Inicial del rango: " ini
            read -p "IP Final: " fin
            read -p "Gateway (presione Enter para usar $srvIP): " gw
            [[ -z "$gw" ]] && gw=$srvIP
            
            # DNS Primario será la IP del servidor
            dns=$srvIP
            echo -e "\e[90m[i] DNS Primario configurado automaticamente: $dns\e[0m"
            
            read -p "DNS Secundario (presione Enter para 8.8.8.8): " dns2
            [[ -z "$dns2" ]] && dns2="8.8.8.8"
            ...
            if validar_ip $ini && validar_ip $fin; then
                ...
                if systemctl is-active --quiet dhcpd; then
                    ...
                    echo -e "\e[32m[+] Gateway: $gw\e[0m"
                    echo -e "\e[32m[+] DNS Primario: $dns\e[0m"
                    echo -e "\e[32m[+] DNS Secundario: $dns2\e[0m"
